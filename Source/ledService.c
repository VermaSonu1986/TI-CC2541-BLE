...
static uint8 LedDataCharValue[LED_DATA_CHAR_LEN] = {0};
  ...
  if ( pAttr->type.len == ATT_UUID_SIZE )
  {
    ...
    if (osal_memcmp(pAttr->type.uuid, LedDataCharUUID, ATT_UUID_SIZE)) {
      ...
      {               
        ...
        if (pAttr->pValue[0] == 0x01)          
          HalLedSet(HAL_LED_1,HAL_LED_MODE_ON);         //ON LED
        else
          HalLedSet(HAL_LED_1,HAL_LED_MODE_OFF);         //OFF_LED
